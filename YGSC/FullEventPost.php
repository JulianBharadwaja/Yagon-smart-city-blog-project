...
if (isset($_POST["Submit"])) {
  ...
  if (empty($Name) || empty($Email) || empty($Comment)) {
    ...
    Redirect_to("FullEventPost.php?id=$PostIdFromURL");
    ...
    $sql = "INSERT INTO comments_events(datetime,name,email,comment,approvedby,status,event_id)";
  ...
  <main>
    ...
    <section class="container">
      <div class="site-content">
        <div class="posts">
          <!-- RETRIVING DATA FROM THE EVENT TABLE TO FETCH THE ID WHICH HAS THE SAME ID AS REQUESTED ID -->
          <?php
          // CHECKING SESSION MESSAGE FROM FULL POST FOR VALIDATION
          echo ErrorMessage();
          echo SuccessMessage();
          $sql = "SELECT * FROM events WHERE id='$PostIdFromURL'";
          $stmt = $con->query($sql);
          ...
          ?>
            ...
            <div class="post-content" data-aos="zoom-in" data-aos-delay="200">
              ...
              <div class="post-title">
                <p>
                  <!-- CONDITIONAL STATEMENT FOR THE CHECKING WHETHER IT IS THE ONLINE EVENT OR ELSE -->
                  <?php
                  if ($eventType == 'Online Event') {
                  ?>
                    <i class="fas fa-video"></i> <?php echo htmlentities($eventType); ?>
                    <p>Link: <a href="<?php echo htmlentities($Location); ?>" target="_blank" rel="noopener noreferrer"><?php echo htmlentities($Location); ?></a></p>
                  <?php
                  } else {
                  ?>
                    <i class="fas fa-street-view"></i> <?php echo htmlentities($eventType); ?>
          ...
          <?php
          global $con;
          $sql = "SELECT * FROM comments_events WHERE event_id='$PostIdFromURL' AND status='ON'";
          $stmt = $con->query($sql);
          while ($DataRows = $stmt->fetch()) {
          ...
          <form action="FullEventPost.php?id=<?php echo $PostIdFromURL; ?>" method="post">
